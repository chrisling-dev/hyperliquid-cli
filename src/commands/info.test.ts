...
describe("info commands", () => {
  ...
  describe("prices command logic", () => {
    it("should return all prices from HTTP when no server", async () => {
      const mids = { BTC: "50000", ETH: "3000" }
      mockInfoClient.allMids.mockResolvedValue(mids)

      // Simulate the prices command logic
      const serverClient = await mockContext.getServerClient()
      expect(serverClient).toBeNull()

      const client = mockContext.getPublicClient()
      const result = await client.allMids()
      expect(result).toEqual(mids)
    })

    it("should return prices from server cache when available", async () => {
      const cachedData = { BTC: "50000", ETH: "3000" }
      mockServerClient.getPrices.mockResolvedValue({
        data: cachedData,
        cached_at: Date.now(),
      })
      ...
      serverClient!.close()
      expect(mockServerClient.close).toHaveBeenCalled()
    })

    it("should fall back to HTTP if server fails", async () => {
      mockServerClient.getPrices.mockRejectedValue(new Error("Server error"))
      mockContext.getServerClient.mockResolvedValue(mockServerClient)

      const httpData = { BTC: "51000" }
      mockInfoClient.allMids.mockResolvedValue(httpData)
      ...
      try {
        ...
        serverClient!.close()
        result = await mockContext.getPublicClient().allMids()
      }

      expect(result).toEqual(httpData)
    })

    it("should filter by coin when --pair is provided", () => {
      const mids = { BTC: "50000", ETH: "3000", SOL: "100" }
      const pair = "btc"
      const coin = pair.toUpperCase()
      const price = mids[coin as keyof typeof mids]

      expect(price).toBe("50000")
      expect({ coin, price }).toEqual({ coin: "BTC", price: "50000" })
    })

    it("should handle unknown coin in --pair filter", () => {
      const mids = { BTC: "50000", ETH: "3000" }
      const pair = "UNKNOWN"
      const price = mids[pair as keyof typeof mids]
